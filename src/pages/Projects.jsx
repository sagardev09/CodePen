        ...
        if (searcterm?.length > 0) {
            setfiltered(
                projects?.filter((item) => {
                    ...
                    return searcterm.split("").every((letter) => lowercase.includes(letter))
                })
            )
        }
        else {
                    ...
                    filtered ? <>
                        {filtered && filtered.map((item, index) => (
                            <motion.div
                                ...
                                key={item.id} className="h-[400px] w-full cursor-pointer md:w-[450x] rounded-lg  flex flex-col items-center justify-center gap-2 bg-secondary"
